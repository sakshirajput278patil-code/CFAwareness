...
def test_calculate_total_carbon_footprint_average():
    input_data = CarbonCalculationInput(
        ...
        consumption=ConsumptionInput(consumption_type=ConsumptionType.AVERAGE) # 2500.0
    )
    ...
    total_expected = round(transport_expected + energy_expected + diet_expected + consumption_expected, 2)
    ...
    # Check percentages
    assert result.global_average_comparison_percent == round((total_expected / 4000.0) * 100, 2)
    assert result.paris_target_comparison_percent == round((total_expected / 2000.0) * 100, 2)
