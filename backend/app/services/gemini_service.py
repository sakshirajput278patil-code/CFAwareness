...
def generate_deterministic_insights(breakdown: CarbonCalculationBreakdown) -> List[str]:
    ...
    if breakdown.transport_kg_co2e > 1000:
        insights.append("Switching 2 car commutes/week to transit could cut ~310 kg CO2e/year.")
    
    # Analyze diet
    if breakdown.diet_kg_co2e > 1500:
        insights.append("Replacing meat with plant-based meals 3 days a week could reduce your emissions by ~400 kg CO2e/year.")
    
    # Analyze energy
    if breakdown.energy_kg_co2e > 1200:
        insights.append("Lowering your thermostat by 1°C and upgrading to LED bulbs could save ~150 kg CO2e/year.")
    
    # Default fallbacks if none trigger
    if len(insights) < 3:
        insights.append("Buying second-hand instead of new clothing can save ~200 kg CO2e/year.")
    if len(insights) < 3:
...
async def generate_personalized_insights(breakdown: CarbonCalculationBreakdown) -> List[str]:
    """
    ...
    try:
        ...
        # Enforce 3s timeout
        response = await asyncio.wait_for(model.generate_content_async(prompt), timeout=3.0)
